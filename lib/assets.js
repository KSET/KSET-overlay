...
function simpleStaticFile(name, startFile, endFile = null) {
    ...
    const type = startFile.split('.').pop();

    const opts = {
        name,
        file: path.resolve(__dirname, `../static/${type}/${startFile}`),
        out: path.resolve(__dirname, `../public/${type}/${endFile}`),
        link: `${baseUrl}/${type}/${endFile}`,
    };
